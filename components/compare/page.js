...
export default async function ComparePage({
  ...
  const [products, allProducts] = await Promise.all([productIds.length > 0 ? Database.getComparisonProducts(productIds) : [], Database.getProducts()]);
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {products.length === 0 ? <EmptyCompareCard products={allProducts} /> : <ComparisonTable products={products} />}
      </div>
    </div>;
}