...
export class Database {
  ...
  static async searchProductsAdvanced(query, filters = {}) {
    try {
      ...
      let products = (await apiClient.searchProducts(query)) || [];
      const normalizedFilters = {
        category: filters.category,
        brand: filters.brand,
        priceRange: filters.priceRange || (filters.minPrice !== undefined || filters.maxPrice !== undefined
          ? {
      ...
      products = this.filterProductsLocal(products, normalizedFilters);
      if (filters.sortBy) {
        products = this.sortProductsLocal(products, filters.sortBy, filters.sortOrder);
      }
      return products;
    } catch (error) {
      console.error("Error searching products:", error);
