...
export function parseNaturalLanguageQuery(query: string): NaturalLanguageQuery {
  ...
  const category = categories.find(
    (cat) =>
      ...
      (cat === "smartphone" && (lowerQuery.includes("phone") || lowerQuery.includes("mobile"))),
  )
  ...
  if (priceMatches) {
    ...
    priceMatches.forEach((match) => {
      if (match.includes("under") || match.includes("below") || match.includes("less than") || match.includes("<")) {
  ...
  return {
    ...
    category,
    brand,
    priceRange: priceRange && (priceRange.min || priceRange.max) ? priceRange : undefined,
